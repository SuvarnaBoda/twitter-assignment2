...
app.get("/user/following/", authenticateToken, async (request, response) => {
  ...
  const query2 = `
        ...
        INNER JOIN follower.following_user_id=tweet.user_id
        WHERE follower_user_id='${user_id}';
      `;
  const userFollowsArray = await db.all(query2);
...
app.get("/user/followers/", authenticateToken, async (request, response) => {
  ...
  const query3 = `
    SELECT name FROM user 
    INNER JOIN user.user_id=follower.follower_user_id
    WHERE follower_user_id='${user_id}';
  `;
  const userresponse = await db.all(query3);
